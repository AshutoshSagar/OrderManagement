...
function OrderForm({ userId, handleOrderSuccess }) {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          `https://${url}.execute-api.us-west-2.amazonaws.com/v1/get-products`
        );
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      const response = await axios.post(`https://${url}.execute-api.us-west-2.amazonaws.com/v1/place-order`, {
        user_id: userId,
